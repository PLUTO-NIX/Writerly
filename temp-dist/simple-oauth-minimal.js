...
const AdvancedSlackParser_1 = require("./parsers/AdvancedSlackParser");
const FormatDetector_1 = require("./formatters/FormatDetector");
const FormatAwarePrompts_1 = require("./prompts/FormatAwarePrompts");
// Firestore 기반 인증 서비스
const firestore_auth_service_1 = require("./services/firestore-auth.service");
// Thread Support - Slack Events API 핸들러
const slack_events_handler_1 = require("./handlers/slack-events.handler");
...
const advancedParser = new AdvancedSlackParser_1.AdvancedSlackParser();
const formatDetector = new FormatDetector_1.FormatDetector();
const formatAwarePrompts = new FormatAwarePrompts_1.FormatAwarePrompts();
// Thread Support - Events API 핸들러 인스턴스
const slackEventsHandler = new slack_events_handler_1.SlackEventsHandler();
// OAuth configuration
...
async function sendBotMessage(channel, text, authUrl) {
    if (!SLACK_BOT_TOKEN) {
...
async function sendEphemeralProcessingMessage(channel, userId, text) {
    if (!SLACK_BOT_TOKEN) {
...
async function deleteMessage(channel, messageTs) {
    if (!SLACK_BOT_TOKEN) {
...
async function sendUserMessage(channel, text, userToken) {
    try {
        const response = await fetch('https://slack.com/api/chat.postMessage', {
            method: 'POST',
            headers: {
...
async function isUserAuthenticated(userId, teamId) {
    return await firestore_auth_service_1.authService.isAuthenticated(userId, teamId);
}
// Get user OAuth token
async function getUserToken(userId, teamId) {
    return await firestore_auth_service_1.authService.getAuth(userId, teamId);
}
// Store user session
async function storeUserSession(userId, teamId, accessToken) {
    await firestore_auth_service_1.authService.storeAuth(userId, teamId, accessToken);
}
// OAuth start
...
app.get('/health/auth', async (req, res) => {
    try {
        // Firestore 연결 테스트
        const testDoc = await firestore_auth_service_1.authService.firestoreDB
            .collection('health')
            .doc('check')
            .get();
        const cacheStats = firestore_auth_service_1.authService.getCacheStats();
        res.json({
            status: 'healthy',
            firestore: 'connected',
            cache: cacheStats,
...
function parseSlashCommandAdvanced(text) {
    const fullCommand = `/ai ${text}`;
    const parsed = advancedParser.parse(fullCommand);
    if (advancedParser.isValidParse(parsed)) {
...
AI 응답이 사용자 이름으로 표시됩니다.`
        ...
        if (text.trim().toLowerCase() === 'logout' || text.trim() === '로그아웃') {
            await firestore_auth_service_1.authService.deleteAuth(user_id, team_id);
            return res.json({
                response_type: 'ephemeral',
...
async function processAIRequestWithFormatPreservation(parsedCommand, userId, channelId, teamId) {
    try {
...
async function processAIRequestWithFormatPreservationAndTracking(parsedCommand, userId, channelId, teamId, res) {
    let processingMessageTs = null;
    try {
        ...
        const complexityMessage = parsedCommand.metadata.complexity === 'complex'
            ...
            : '';
        processingMessageTs = await sendEphemeralProcessingMessage(channelId, userId, `🤖 AI가 요청을 처리하고 있습니다... 잠시만 기다려주세요!\n\n✨ 완료되면 사용자 이름으로 응답이 표시됩니다.${complexityMessage}`);
...
async function processAIRequestWithUserToken(prompt, data, userId, channelId, teamId) {
    try {
        const userToken = await getUserToken(userId, teamId);
        if (!userToken) {
            console.error('User token not found:', { userId, teamId });
...
async function processAIRequestWithUserTokenAndTracking(prompt, data, userId, channelId, teamId, res) {
    let processingMessageTs = null;
    try {

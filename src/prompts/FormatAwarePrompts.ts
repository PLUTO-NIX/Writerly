...
export class FormatAwarePrompts {
   ...
   */
  generateFormatPreservingPrompt(config: PromptConfig): string {
...
Important: Provide only the final result without any explanatory text, labels, or meta-commentary. Do not repeat the original content or task instruction.`;
   ...
   * 작업 지시사항을 명확한 영어로 변환
   */
  getTaskInstruction(task: string): string {
   ...
   * 작업별 특화된 전략 생성
   */
  getTaskSpecificStrategy(task: string, metadata: FormatMetadata): PromptStrategy {
    const lowerTask = task.toLowerCase();
   ...
   * 문법 검토 전략
   */
  getGrammarStrategy(metadata: FormatMetadata): PromptStrategy {
    return {
   ...
   * 설명/분석 전략
   */
  getExplanationStrategy(metadata: FormatMetadata): PromptStrategy {
    return {
   ...
   * 기본 전략
   */
  getDefaultStrategy(metadata: FormatMetadata): PromptStrategy {
    return {
   ...
   * 기본 작업 지시사항 생성
   */
  getBaseInstruction(task: string): string {
   ...
   */
  buildFormatInstructions(metadata: FormatMetadata): string {
   ...
   * 보존 레벨별 지시사항
   */
  getPreservationInstructions(level: 'strict' | 'adaptive' | 'enhanced'): string {
    switch (level) {
      case 'strict':
...
- Optimize for Slack display`;
   ...
   * 서식별 세부 규칙 생성
   */
  getFormattingRules(metadata: FormatMetadata, level: string): string {
    const rules = [];
   ...
   * 복잡도별 처리 전략
   */
  getComplexityStrategy(complexity: string): string {
    switch (complexity) {
      case 'simple':
   ...
   * 디버깅용 프롬프트 정보 출력
   */
  getPromptInfo(config: PromptConfig): string {
    return `
프롬프트 생성 정보:
- 작업: ${config.task}

...
export class MentionParser {
      ...
      if (match && match[1] && match[2]) {
        ...
        if (this.validateLength(task, data)) {
          return {
            ...
            data,
            confidence: 0.95,
            formatMetadata: formatDetector.detectSlackMarkdown(data),
  ...
  private calculateNaturalConfidence(task: string, data: string): number {
  ...
  private validateLength(task: string, data: string): boolean {
    // 작업명 길이 검증
  ...
  public validateParsedCommand(command: ParsedCommand): boolean {
    ...
    if (!this.validateLength(command.task, command.data)) return false;
   ...
   * 파싱 결과 디버깅 정보 제공
   */
  public getParsingInfo(command: ParsedCommand): object {
    return {
      task: command.task,
